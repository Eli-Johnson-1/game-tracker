...
router.post('/games/:id/complete',
  requireAuth,
  param('id').isInt({ min: 1 }),
  body('generation').optional({ nullable: true }).isInt({ min: 1, max: 30 }).withMessage('generation must be 1–30'),
  ...
  body('players.*.mega_credits').optional().isInt({ min: 0 }).withMessage('mega_credits must be ≥ 0'),
